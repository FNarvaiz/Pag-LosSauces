...
function renderDownloads(categoryId)
  ...
  <div id="downloadsListingHeading">
    <div id="downloadControls">
      ...
      <%
        ...
        dbGetData("SELECT C.ID, C.NOMBRE " & _
          ...
          "WHERE C.VIGENTE = 1 AND EXISTS (SELECT * FROM LINEA_TIEMPO L WHERE L.ID_CATEGORIA=C.ID AND L.APROBADO=1) AND " & _
    ...
    <table cellpadding="3" cellspacing="0" width= "100%">
      <tr>
        <td width="80" align="center">PERIODO</td>
...
function renderDownloadListing(categoryId)
  ...
  <table cellpadding="3" cellspacing="0" width= "100%">
    ...
    if dbGetData("SELECT L.ID, L.NOMBRE, L.PERIODO, L.CONTENIDO_TEXTO, L.ARCHIVO_FILENAME, L.ARCHIVO_FILESIZE, C.NOMBRE AS CATEGORIA " & _
        ...
        "WHERE C.VIGENTE = 1 AND L.APROBADO=1 AND " & _
          "(C.SOLO_ADMINISTRACION = 0 OR EXISTS(SELECT * FROM VECINOS V WHERE V.PERMISO_ADMINISTRACION = 1 AND V.ID=" & usrId & ")) " & s & _
        " ORDER BY L.ANIO DESC, L.MES DESC, L.ID DESC") then
