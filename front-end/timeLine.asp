...
function renderTimeLineRowLabels
  if dbGetData("SELECT ID, NOMBRE FROM CATEGORIAS_LINEA_TIEMPO C WHERE C.VIGENTE = 1 AND C.MOSTRAR = 1 AND " & _
    "(C.SOLO_ADMINISTRACION = 0 OR EXISTS(SELECT * FROM VECINOS V WHERE V.PERMISO_ADMINISTRACION = 1 AND V.ID=" & usrId & "))" & _
    "ORDER BY ID") then
    %>
...
function getTimeLineData
  if dbGetData("SELECT L.ANIO, L.MES, L.ID_CATEGORIA FROM LINEA_TIEMPO L " & _
      ...
      "WHERE L.APROBADO=1 AND C.VIGENTE = 1 AND C.MOSTRAR = 1 AND " & _
        "(C.SOLO_ADMINISTRACION = 0 OR EXISTS(SELECT * FROM VECINOS V WHERE V.PERMISO_ADMINISTRACION = 1 AND V.ID=" & usrId & "))" & _
...
function getCategoryQty
  dbGetData("SELECT COUNT(*) FROM CATEGORIAS_LINEA_TIEMPO C WHERE C.VIGENTE = 1 AND C.MOSTRAR = 1 AND " & _
    "(C.SOLO_ADMINISTRACION = 0 OR EXISTS(SELECT * FROM VECINOS V WHERE V.PERMISO_ADMINISTRACION = 1 AND V.ID=" & usrId & "))")
  getCategoryQty = rs(0)
  dbReleaseData
end function
...
function renderTimeLineColumnEnding(row, visibleRows)
  ...
  for i = row - 1 to uBound(visibleRows, 2)
    if visibleRows(0, i) then renderTimeLineNullIcon
  next
  %></table></div></td><%
end function

function renderTimeLineNullIcon
  %>
  <tr>
...
function renderTimeLineGrid(timeLineData, rowQty)
  if uBound(timeLineData) < 0 then
    ...
    <table cellpadding="0" cellspacing="0">
      <tr>
      <%
        dbGetData("SELECT CAST(CASE WHEN C.VIGENTE = 1 AND C.MOSTRAR = 1 THEN 1 ELSE 0 END AS BIT) FROM CATEGORIAS_LINEA_TIEMPO C " & _
          "WHERE (C.SOLO_ADMINISTRACION = 0 OR EXISTS(SELECT * FROM VECINOS V WHERE V.PERMISO_ADMINISTRACION = 1 AND V.ID=" & usrId & ")) " & _
          "ORDER BY C.ID")
        dim visibleRows: visibleRows = rs.getRows
        dbReleaseData
        dim monthNames: monthNames = eval("monthNames" & lang)
        dim i, colKeyValues, iconRow
        dim currentColKeyValues: currentColKeyValues = ""
        dim row: row = 0
        dim column: column = 0
        dim imgId
        ...
        for i = 0 to uBound(timeLineData, 2)
          ...
          iconRow = timeLineData(2, i)
          if currentColKeyValues <> colKeyValues then
            if row > 0 then renderTimeLineColumnEnding row, visibleRows
            currentColKeyValues = colKeyValues
            row = 1
            column = column + 1
            renderTimeLineColumnBeginning column, monthNames(timeLineData(1, i) - 1) & "<br>" & timeLineData(0, i)
            lastColumnTopImgId = ""
          end if
          do while row < iconRow
            if visibleRows(0, row - 1) then renderTimeLineNullIcon
            row = row + 1
          loop
          imgId = "timeLineIcon" & iconRow & "_" & column
...
function renderTimeLine
  ...
  <div id="timeLineContainer">
    <div id="timeLineArrows">
      <div id="timeLineEndLeftArrow" onclick="scroll('timeLineGridContainer', 'right', 1000000)"></div>
      <div id="timeLineFastLeftArrow" onclick="scroll('timeLineGridContainer', 'right', 700)"></div>
      <div id="timeLineLeftArrow" onclick="scroll('timeLineGridContainer', 'right', 70)"></div>
      <div id="timeLineRightArrow" onclick="scroll('timeLineGridContainer', 'left', 70)"></div>
      <div id="timeLineFastRightArrow" onclick="scroll('timeLineGridContainer', 'left', 700)"></div>
      <div id="timeLineEndRightArrow" onclick="scroll('timeLineGridContainer', 'left', 1000000)"></div>
    </div>
    ...
    <div id="timeLineDetail">
      ...
      <div id="timeLineDetailLeftArrow" onclick="scroll('timeLineDetailIcons', 'right', 750)"></div>
      <div id="timeLineDetailRightArrow" onclick="scroll('timeLineDetailIcons', 'left', 750)"></div>
...
dim recognisedFileTypes: recognisedFileTypes = array("dwg", "doc", "docx", "ppt", "xls", "xlsx", "pdf", "zip", "rar")
